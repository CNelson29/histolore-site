# Curious History — pasos para poner en vivo
...
- Sitio Astro completo y rebrandeado (`sites/curioushistory/`), dominio placeholder `curioushistory.net`.
...
- Rama `curioushistory` en `content_research.build_research_brief()`.
...
1. **Dominio**: decidir el real (o quedarte con curioushistory.net). Si cambia, buscar/reemplazar
   `curioushistory.net` en `sites/curioushistory/` y `site_url` en projects.yaml.
2. **Repo GitHub**: crear `CNelson29/curioushistory-site` (o el nombre que elijas). Luego:
   ```
   cd sites/curioushistory
   git init && git add -A && git commit -m "init curious history"
   git remote add origin git@github.com:CNelson29/curioushistory-site.git   # SSH (key Ubuntu Incubadora)
...
4. **Agente**: crear `agents/history_agent.py` = clon de `blog_agent.py` con:
   - `SITE_DIR = .../sites/curioushistory`
   - topics desde `config/history_topics.yaml`
   - `from agents.prompt_factory import history_prompt` y `build_research_brief(site="curioushistory", ...)`
...
5. **Wiring en orquestador** (`agents/orchestrator.py`):
   - `PROJECTS["blog_history"] = {"name": "Curious History", "agent": "history_agent", "active": True}`
